  ...
  BasicStepHandler()
    ...
    .action(/.+/, async (ctx) => {
      if (ctx.session.parents.map((x) => x.id).includes(+ctx.match[0])) {
        ...
        const brends = ctx.session.catalog.filter(
          (p) => p.parent_category == id
        );
        if (brends.length == 0) {
          ...
          ctx
            .editMessageText(
              ...
              Markup.inlineKeyboard(ArrayConcat(brends, ctx), {
                columns: 2,
              }).extra()
            )
            .then((v) => (ctx.session.message_id = v.message_id));
    ...
    .action(match("back"), async (ctx) => {
      ...
      if (no_brand) {
        ...
        ctx
          .reply(
            ctx.i18n.t("choose-type-product"),
            Markup.inlineKeyboard(ArrayConcat(ctx.session.brends, ctx), {
              columns: 2,
            }).extra()
          )
          .then((v) => (ctx.session.message_id = v.message_id));
