...
module.exports = new WizardScene(
  'order',
  async ctx => {
    ctx.reply(ctx.i18n.t('enter-address'),Markup.keyboard([
  ...
  BasicComandHandler()
    ...
    .on('location', async ctx => {
      ...
      const distance = geolib.getDistance({latitude, longitude}, {latitude:41.317104, longitude: 69.283132})
      ...
      ctx.replyWithMarkdown(ctx.i18n.t('choose-payment-method'),
  ...
  BasicComandHandler()
    .action(match('back'), async ctx => {
      ...
      ctx.reply(ctx.i18n.t('enter-address'),Markup.keyboard([
        [Markup.locationRequestButton(ctx.i18n.t('my-location'))],
        [ctx.i18n.t('menu')]
      ]).oneTime().resize().extra()).then(val => ctx.session.message_id = val.message_id)
      ctx.wizard.back()
    })
  ...
  BasicComandHandler()
    ...
    .action(match('cancel'), async ctx => {
      const btn = Markup.callbackButton
      const l = ctx.i18n
      await ctx.editMessageText(ctx.i18n.t('choose-payment-method'),
      Extra.
      markdown()
      .markup(
        Markup.inlineKeyboard([
    ...
    .action(match('confirm'), async ctx => {
      try {
        ...
        await axios.post('http://botmasters.uz/api/orders/new_order', {key: 'new_order'})
        await ctx.deleteMessage(ctx.session.message_id)
        await ctx.replyWithMarkdown(ctx.i18n.t('will-call-you', {id: order.dataValues.id}))
        ctx.session.cart = []
        ctx.session.in_cart = 0
