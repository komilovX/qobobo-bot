...
function keyboard(ctx) {
  ...
  const l = ctx.i18n
  return [
    [btn(l.t('change-name'), l.t('change-name')), btn(l.t('change-contact-phone'), l.t('change-contact-phone'))],
...
module.exports = new WizardScene(
  ...
  async ctx => {
    ...
    ctx.editMessageText(ctx.i18n.t('settings'), Markup.inlineKeyboard(
      keyboard(ctx)
  ...
  .action(match('change-name'), async ctx => {
    ...
    ctx.editMessageText(ctx.i18n.t('your-name',{name: first_name, last: last_name}), Markup.inlineKeyboard([
      [btn(l.t('edit-name'), l.t('edit-name')), btn(l.t('edit-last-name'), l.t('edit-last-name'))],
      [btn(l.t('back'), l.t('back'))]
  ...
  .action(match('change-contact-phone'), async ctx => {
    ...
    ctx.editMessageText(ctx.i18n.t('settings-send-number',{phone}), Markup.inlineKeyboard([
      [btn(l.t('back'), l.t('back'))]
    ]).resize().extra()).then(val => ctx.session.message_id = val.message_id)
  ...
  .action(match('edit-name'), async ctx => {
    await ctx.editMessageText(ctx.i18n.t('enter-new-name'), Markup.inlineKeyboard(
  ...
  .action(match('edit-last-name'), async ctx => {
    await ctx.editMessageText(ctx.i18n.t('enter-new-last-name'), Markup.inlineKeyboard(
      Markup.callbackButton(ctx.i18n.t('back'), ctx.i18n.t('back'))
    ).resize().extra()).then(val => ctx.session.message_id = val.message_id)
    await ctx.answerCbQuery()
  ...
  .hears(/\+998\s\d{2}\s\d{3}\s\d{2}\s\d{2}/g, async ctx => {
    ...
    ctx.reply(ctx.i18n.t('settings'), Markup.inlineKeyboard(
      keyboard(ctx)
    ).resize().extra()).then(val => ctx.session.message_id = val.message_id)
    ctx.wizard.back()
  })
  ...
  .action(match('back'), async ctx => {
    ...
    const l = ctx.i18n
    const { first_name, last_name } = ctx.session
    await ctx.editMessageText(ctx.i18n.t('your-name',{name: first_name, last: last_name}), Markup.inlineKeyboard([
      [btn(l.t('edit-name'), l.t('edit-name')),btn(l.t('edit-last-name'), l.t('edit-last-name'))],
      [btn(l.t('back'), l.t('back'))]
    ]).resize().extra()).then(val => ctx.session.message_id = val.message_id)
    await ctx.answerCbQuery()
  ...
  .on('text', async ctx => {
    ...
    const l = ctx.i18n
    const { first_name, last_name } = ctx.session
    ctx.reply(ctx.i18n.t('your-name',{name: first_name, last: last_name}), Markup.inlineKeyboard([
      [btn(l.t('edit-name'), l.t('edit-name')),btn(l.t('edit-last-name'), l.t('edit-last-name'))],
      [btn(l.t('back'), l.t('back'))]
    ]).resize().extra()).then(val => ctx.session.message_id = val.message_id)
    ctx.wizard.back()
